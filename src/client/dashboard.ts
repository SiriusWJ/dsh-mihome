/**
 * Browser-side Conversation Node for the Mi Home dashboard.
 *
 * Matches `tool/result` events whose `meta` carries the dashboard snapshot
 * (projected by `mi_dashboard` via `presentationMeta`) and renders a keyed
...
declare module '@deepseek-ai/dsh-client-ui-conversation/client' {
  interface ChatNodeDataMap {
    'mihome-dashboard': DashboardChatData
  }
}
...
/**
 * One dashboard call is a single checkpoint event: `mi_dashboard` returns the
...
export const dashboardDefinition: ConversationNodeDefinition<DashboardState> = {
  ...
  start: (_context, match, _reader: ConversationContextReader) => {
    if (match.event.type !== 'tool/result' || !isDashboardMeta(match.event.data.meta)) {
      throw new Error('mihome-dashboard requires a tool/result carrying dashboard meta')
    }
    return {
